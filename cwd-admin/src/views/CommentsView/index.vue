<template>
  <div class="page">
    ...
    <div v-else>
      <!-- Mobile Card Layout -->
      <div v-if="isMobile" class="comment-cards">
        <div v-for="item in filteredComments" :key="item.id" class="comment-card">
          <div class="comment-card-header">
            ...
            <div class="comment-card-meta">
              <div class="comment-card-name">
                ...
                <span v-if="item.isAdmin" class="cell-admin-tag">
                  <svg
                    ...
                    style="width: 14px; height: 14px; fill: currentColor; color: #db850d; vertical-align: -0.15em;"
                  >
                    <g>
              ...
              <div class="comment-card-email">{{ item.email }}</div>
              <div class="comment-card-time">{{ formatDate(item.created) }}</div>
            </div>
            <span class="cell-status" :class="`cell-status-${item.status}`">
              {{ formatStatus(item.status) }}
            </span>
          </div>
          <div class="comment-card-content" v-html="item.contentHtml"></div>
