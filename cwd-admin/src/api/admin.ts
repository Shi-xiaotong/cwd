...
export type LanzouFileItem = {
	...
	lanzou_share_url: string;
	direct_url: string;
	folder_id: string;
	created_at: number;
};

export type LanzouListResponse = {
	data: LanzouFileItem[];
	total: number;
	page: number;
	pageSize: number;
...
export function fetchLanzouList(
	page: number,
	pageSize: number = 20,
	search: string = ''
): Promise<LanzouListResponse> {
	const searchParams = new URLSearchParams();
	searchParams.set('page', String(page));
	searchParams.set('pageSize', String(pageSize));
	if (search) searchParams.set('search', search);
	return get<LanzouListResponse>(`/admin/lanzou/list?${searchParams.toString()}`);
}

export async function uploadLanzouFile(
	file: File,
	coverFile?: File | null,
	folderId?: string
	...
	const res = await fetch(`${apiBaseUrl}/admin/lanzou/upload`, {
		method: 'POST',
		headers: {
			...(token ? { Authorization: `Bearer ${token}` } : {}),
		},
...
export async function uploadLanzouByUrl(
	fileUrl: string,
	folderId?: string,
	maxRetries: number = 2
): Promise<{ message: string; data: any }> {
	...
	for (let attempt = 0; attempt < maxRetries; attempt++) {
		try {
			const res = await fetch(`${apiBaseUrl}/admin/lanzou/upload-url`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					...(token ? { Authorization: `Bearer ${token}` } : {}),
				},
				body: JSON.stringify({ url: fileUrl, folder_id: folderId }),
			});

			let data: any = null;
			try { data = await res.json(); } catch { data = null; }
			if (!res.ok) {
				throw new Error(data?.message || `上传失败，状态码 ${res.status}`);
			}
			return data;
		} catch (e: any) {
			lastError = e;
...
export function deleteLanzouFile(id: number): Promise<{ message: string }> {
	return del<{ message: string }>(`/admin/lanzou/delete?id=${id}`);
}

export function fetchLanzouDetail(id: number): Promise<{ data: LanzouFileItem }> {
	return get<{ data: LanzouFileItem }>(`/admin/lanzou/detail?id=${id}`);
}

export type LanzouSettingsResponse = {
	cookie: string;
	folderId: string;
	hasCookie: boolean;
};

export function fetchLanzouSettings(): Promise<LanzouSettingsResponse> {
	return get<LanzouSettingsResponse>('/admin/lanzou/settings');
}

export function saveLanzouSettings(data: { cookie?: string; folderId?: string }): Promise<{ message: string }> {
	return put<{ message: string }>('/admin/lanzou/settings', data);
}
